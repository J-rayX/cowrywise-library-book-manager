...
class BorrowBookView(APIView):
    def post(self, request, book_id):
        ...
        if not user_email or not borrow_days:
            return JsonResponse(
                {"error": "The [user_email and borrow_days] fields are required"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            ...
            return JsonResponse(
                {"error": "borrow_days value must be a valid integer"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            ...
            return JsonResponse(
                {
                    "error": "Failed to notify the admin about borrowing",
                    "detail": str(e),
                },
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class AvailableBooksView(APIView):
    def get(self, request):
        try:
            ...
            return JsonResponse(
                {"error": "An error occurred while fetching books. ", "detail": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class FilterAllBooksView(APIView):
    def get(self, request):
        ...
        try:
            ...
            if not books_list:
                return JsonResponse(
                    {"message": "No books found with the provided filters"},
                    status=status.HTTP_404_NOT_FOUND,
                )
