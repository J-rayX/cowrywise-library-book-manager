...
def consume_borrowed():
    ...
    def callback(ch, method, properties, body):
        ...
        try:
            ...
            book.save()
            print(
                f"Updated book {book.title} as unavailable and {book.return_date} return_date."
            )
    ...
    channel.basic_consume(
        queue="book_borrowed", on_message_callback=callback, auto_ack=True
    )
...
def consume_user_enrolled():
    ...
    channel.basic_consume(
        queue="user_enrolled", on_message_callback=callback, auto_ack=True
    )
