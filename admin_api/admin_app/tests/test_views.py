...
import pika
from admin_app.models import AdminBorrowing, AdminUser, Book
...
class AddBookViewTests(APITestCase):
    ...
    def test_add_book_success(self, mock_pika):
        ...
        self.assertEqual(
            response_data["book"]["publisher"], self.valid_data["publisher"]
        )
    ...
    def test_add_book_missing_fields(self):
        ...
        self.assertEqual(
            response.json()["error"],
            "The [title, author, publisher, category] fields are required.",
        )

        response = self.client.post(
            self.url,
            {"title": "Title", "publisher": "Publisher", "category": "Category"},
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
    ...
    def test_add_book_title_length_exceed(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            response.json()["error"], "Title must not be above 200 character."
        )
...
class UnavailableBooksViewTests(APITestCase):
    ...
    def test_get_unavailable_books_exception(self, mock_filter):
        ...
        self.assertEqual(
            response.json()["error"],
            "An error occurred while fetching unavailable books.",
        )
...
class AdminUserListViewTests(APITestCase):
    ...
    def test_get_admin_user_list_exception(self):
        with patch("admin_app.views.AdminUser.objects.all") as mock_all:
            ...
            self.assertEqual(
                response_data["error"], "An error occurred while fetching users"
            )
...
class UserBorrowingListViewTests(APITestCase):
    def setUp(self):
        ...
        self.borrowing1 = AdminBorrowing.objects.create(
            user=self.user1, book_id=1, borrow_days=7, return_date="2024-09-30"
        )
        self.borrowing2 = AdminBorrowing.objects.create(
            user=self.user2, book_id=2, borrow_days=14, return_date="2024-10-15"
        )
    ...
    def test_get_user_borrowing_list_success(self, mock_borrowings, mock_users):
        ...
        mock_borrowings.side_effect = (
            lambda user: [self.borrowing1]
            if user == self.user1
            else [self.borrowing2]
            if user == self.user2
            else []
        )
        ...
        user1_data = next(
            (
                ...
                if user["user_email"] == "jkaylight@gmail.com"
            ),
            None,
        )
        self.assertIsNotNone(
            user1_data, "User with email 'jkaylight@gmail.com' not found in response"
        )
        ...
        user2_data = next(
            (
                user
                for user in response_data["users"]
                if user["user_email"] == "jkaylight01@gmail.com"
            ),
            None,
        )
        self.assertIsNotNone(
            user2_data, "User with email 'jkaylight01@gmail.com' not found in response"
        )
